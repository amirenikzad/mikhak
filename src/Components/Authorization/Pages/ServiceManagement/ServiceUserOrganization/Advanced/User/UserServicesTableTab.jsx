...
import { GET_USER_SERVICE_ORGANIZATION } from '../../../../../../Base/UserAccessNames.jsx';
...
export default function UserServicesTableTab({
                                               user_id,
                                               reactQueryItemName,
  ...
  const [allUserService, setAllUserService] = useState([]);
  ...
  const allUserServiceOrganizationAxios = async ({ pageParam = 1 }) => {
    if (user_id && (accessSlice.isAdmin || accessSlice.userAccess?.includes(GET_USER_SERVICE_ORGANIZATION))) {
      try {
        ...
        const response = await fetchWithAxios.get(
          `/user_service_organization?id=${user_id}&page=${pageParam}&page_size=20&search=${searchValue}`,
          {
  ...
  const {
    ...
    queryFn: allUserServiceOrganizationAxios,
    initialPageParam: 1,
  ...
  useEffect(() => {
    if (ServicesData?.pages[0]) {
      setAllUserService(ServicesData?.pages.flatMap((page) => page?.services));
    }
  }, [ServicesData]);
      ...
      <Box dir={'ltr'} position={'relative'}>
        ...
        <TableContainer component={Paper}
          ...
          <Table stickyHeader>
            ...
            <TableBody>
              {stableSort(allUserService, getComparator(order, orderBy)).map((row, index) => (
