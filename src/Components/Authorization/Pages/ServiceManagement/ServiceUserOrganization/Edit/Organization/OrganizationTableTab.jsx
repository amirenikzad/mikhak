...
export default function OrganizationTableTab({
                                               service_id,
                                               reactQueryItemName,
  ...
  const [allApiCategory, setAllApiCategory] = useState([]);
  ...
  const headCells = useMemo(() => [
    { id: 'name', numeric: false, disablePadding: true, label: giveText(131) },
    { id: 'active', numeric: false, disablePadding: false, label: giveText(2) },
  ], []);

  const allCategoryComponentApiAxios = async ({ pageParam = 1 }) => {
    try {
      ...
      const response = await fetchWithAxios.get(
        `/service_user_organization?id=${service_id}&page=${pageParam}&page_size=20&search=${searchValue}&type=organization`,
        {
  ...
  const {
    ...
    hasNextPage,
    isLoading,
    isFetchingNextPage: isFetchingListAllApis,
    isRefetching,
    isError: errorGetAllRoles,
  } = useInfiniteQuery({
    queryKey: [reactQueryItemName, service_id, searchValue],
    queryFn: allCategoryComponentApiAxios,
    initialPageParam: 1,
  ...
  const lastElementRef = useCallback((node) => {
    ...
    observer.current = new IntersectionObserver((entries) => {
      if (entries[0].isIntersecting && hasNextPage && !isFetchingListAllApis && !isRefetching) {
        fetchNextPage().then(() => null);
      }
    });

    if (node) observer.current.observe(node);
  }, [fetchNextPage, hasNextPage, isFetchingListAllApis, isLoading, isRefetching]);

  useEffect(() => {
    if (apiData) {
      setAllApiCategory(apiData?.pages.flatMap((page) => page?.organizations));
    }
  }, [apiData]);
      ...
      <Box dir={'ltr'} position={'relative'}>
        ...
        <TableContainer component={Paper}
          ...
          <Table stickyHeader>
            ...
            <TableBody>
              {stableSort(allApiCategory, getComparator(order, orderBy)).map((row, index) => (
                <OrganizationTable key={row?.service_id}
                                   index={index}
                                   row={row}
          ...
          <ScrollToPaginate isLoading={isFetchingListAllApis} lastElementRef={lastElementRef} hasPagination={true} />
        </TableContainer>
      </Box>
    </Box>
  );
