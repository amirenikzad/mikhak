...
export const useruseTableBaseActions = ({
                                      onCloseEscape,
                                      onShiftN,
                                      onShiftA,
                                      getAllURL,
  ...
  const allTableListAxios = async ({ pageParam = 1 }) => {
    if (loadWhenIsTrue) {
      ...
      const fullParams = { ...baseParams, ...additionalParams };

      const queryString = qs.stringify(fullParams, { skipNulls: true });

      try {
        const response = await fetchWithAxios.get(
          `${getAllURL}?${queryString}${queryParameter}`,
          {
  ...
  const queryDeps = useMemo(() => {
    return [loadWhenIsTrue, useQueryDependsUpdate, searchValue, queryParameter, JSON.stringify(additionalParams)];
